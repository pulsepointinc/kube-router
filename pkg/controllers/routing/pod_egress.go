...
func (nrc *NetworkRoutingController) deletePodEgressRules() error {
	...
	for _, args := range podEgressArgs {
		...
		if err != nil {
			lastError = errors.New("Failed to lookup iptable rule for outbound traffic from pods: " + err.Error())
			continue
		}
		...
		if exists {
			...
			if err != nil {
				lastError = errors.New("Failed to delete iptable rule for outbound traffic from pods: " +
...
func (nrc *NetworkRoutingController) deleteBadPodEgressRules() error {
	...
	for _, args := range podEgressArgsBad {
		...
		if exists {
			err = iptablesCmdHandler.Delete("nat", "POSTROUTING", args...)
			if err != nil {
				lastError = fmt.Errorf("Failed to delete old/bad iptable rule to "+
